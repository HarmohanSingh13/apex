...
`));
    ...
    toInstall = ["agents"];
    console.log(source_default.yellow("  No agent directories detected. Installing generic .agents/ workflows."));
    console.log(source_default.dim("  Tip: run with --all to install adapters for all supported agents.\n"));
  }
  ...
  for (const provider of toInstall) {
    ...
    if (result.installed > 0) {
      const icon = options.dryRun ? source_default.cyan("  \u25CB") : source_default.green("  \u2713");
      console.log(`${icon} ${source_default.bold(config.label)}`);
  ...
  console.log("");
  if (options.dryRun) {
    console.log(source_default.cyan("  Dry run complete. No files were written.\n"));
    return;
  }
  if (installedCount > 0) {
...
program2.name("ai-sdlc").description("Enterprise AI-SDLC Framework \u2014 install agent workflows into any project").version(pkg.version);
program2.command("install").description("Install AI-SDLC workflows into the current project").option("--all", "Install for all supported agents, even if not auto-detected").option("--dry-run", "Preview what would be installed without writing files").action(installCommand);
program2.action(() => {
